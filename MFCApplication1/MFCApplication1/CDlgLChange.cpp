...
IMPLEMENT_DYNAMIC(CDlgLChange, CDialogEx)

CDlgLChange::CDlgLChange(CWnd* pParent /*=nullptr*/)
...
CDlgLChange::~CDlgLChange()
{
}

void CDlgLChange::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgLChange, CDialogEx)
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_WM_PAINT()
...
void CDlgLChange::OnLButtonDown(UINT nFlags, CPoint point)
{
	// 当用户单击鼠标左键开始拖动
...
void CDlgLChange::OnMouseMove(UINT nFlags, CPoint point)
{
	// 判断当前光标是否在绘制区域
	if (m_MouseRect.PtInRect(point))
	{
...
void CDlgLChange::OnLButtonUp(UINT nFlags, CPoint point)
{
	// 当用户释放鼠标左键停止拖动
	if (m_iIsDraging != 0)
	{
