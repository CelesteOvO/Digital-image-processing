...
IMPLEMENT_DYNAMIC(CDlgVChangeReal, CDialogEx)

CDlgVChangeReal::CDlgVChangeReal(CWnd* pParent /*=nullptr*/)
...
CDlgVChangeReal::~CDlgVChangeReal()
{
}

void CDlgVChangeReal::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgVChangeReal, CDialogEx)
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_WM_PAINT()
...
BOOL CDlgVChangeReal::OnInitDialog()
{
	// 调用默认OnInitDialog函数
...
void CDlgVChangeReal::OnLButtonDown(UINT nFlags, CPoint point)
{
	// 当用户单击鼠标左键开始拖动
...
void CDlgVChangeReal::OnMouseMove(UINT nFlags, CPoint point)
{
	// 判断当前光标是否在绘制区域
	if (m_MouseRect.PtInRect(point))
	{
...
void CDlgVChangeReal::OnLButtonUp(UINT nFlags, CPoint point)
{
	// 当用户释放鼠标左键停止拖动
	if (m_iIsDraging != 0)
	{
