...
IMPLEMENT_DYNAMIC(CDlgVChange, CDialogEx)

CDlgVChange::CDlgVChange(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_InverseVchange, pParent)
{
	m_bX1 = 127;
	m_bY1 = 0;
}

CDlgVChange::~CDlgVChange()
{
}

void CDlgVChange::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlgVChange, CDialogEx)
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONUP()
	ON_WM_PAINT()
...
void CDlgVChange::OnLButtonDown(UINT nFlags, CPoint point)
{
	// 当用户单击鼠标左键开始拖动
...
void CDlgVChange::OnMouseMove(UINT nFlags, CPoint point)
{
	// 判断当前光标是否在绘制区域
	if (m_MouseRect.PtInRect(point))
	{
...
void CDlgVChange::OnLButtonUp(UINT nFlags, CPoint point)
{
	// 当用户释放鼠标左键停止拖动
	if (m_iIsDraging != 0)
	{
